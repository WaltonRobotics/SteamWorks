...
public class AutonEncoderWithVision extends CommandGroup {
	...
	private void autonSequence() {
		...
		if (!(found = hasFoundPeg())) {
			...
			if (!(found = hasFoundPeg())) {
				Robot.drivetrain
						.addControllerMotion(new MotionPathTurn(pose, PEG_ANGLE * 2, MAX_SPEED, MAX_ACCELERATION));
				found = hasFoundPeg();
			}
		}
	...
	public boolean hasFoundPeg() {
		return !(SmartDashboard.getString("status peg", "").equals("too few contours")
				&& !SmartDashboard.getBoolean("Valid angle peg", false));
	}

	public boolean hasFoundGoal() {
		return !(SmartDashboard.getString("status goal", "").equals("too few contours"));
	}

	public void moveForward() {
		Robot.drivetrain.addControllerMotion(new MotionPathStraight(pose, 2.54, MAX_SPEED, MAX_ACCELERATION));
	}

	private void movingToPeg(boolean found) {

		double distance = SmartDashboard.getNumber("Camera distance peg", 0) * 2.54 / 100;
		double angle = SmartDashboard.getNumber("Target angle peg", 0);
	...
	private void moveToBoiler() {
		boolean found = false;
		
		//70 should be changed as it was put randomly. Units are in inches and converted to meters.
		Robot.drivetrain
				.addControllerMotion(new MotionPathStraight(pose, 70 * 2.54 / 100, MAX_SPEED, MAX_ACCELERATION));

		if (!(found = hasFoundPeg())) {
			Robot.drivetrain.addControllerMotion(new MotionPathTurn(pose, -BOILER_ANGLE, MAX_SPEED, MAX_ACCELERATION));

			if (!(found = hasFoundPeg())) {
				Robot.drivetrain
						.addControllerMotion(new MotionPathTurn(pose, BOILER_ANGLE * 2, MAX_SPEED, MAX_ACCELERATION));
				found = hasFoundPeg();
			}
		}
