...
public class DriveSplineByEncoder extends Command {
  ...
  public DriveSplineByEncoder(boolean isDashboard, double distance, double speed,
  ...
  protected void initialize() {
    ...
    /*
     * System.out.println(String.format("Distance=%f, Speed=%f, Accel=%f", distance, speed,
     * acceleration)); motionFinished = false; Pose init = poseEstimator.getPose(); Pose final_;
     * if(isForwards){ final_ = new Pose(new Point2D(init.X.x + distance, init.X.y + distance),
     * angle); } else{ final_ = new Pose(new Point2D(init.X.x - distance, init.X.y - distance),
     * angle); } System.out.println(final_.toString()
     */
  ...
  /*
   * public MotionProvider getAutonMotion(){ Pose init = poseEstimator.getPose(); Pose final_ = new
   * Pose(new Point2D(-2.34, -1.422), Math.PI / 3); return new MotionPathSpline(init, 1.58, final_,
   * 0.95, false); }
   */
  @Override
  ...
  public String toString() {
    return String.format("acceleration = %f,distance = %f,speed = %f", acceleration, distance,
        speed);
  }
}
