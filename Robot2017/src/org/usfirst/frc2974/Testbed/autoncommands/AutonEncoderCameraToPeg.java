...
public class AutonEncoderCameraToPeg extends Command {
  ...
  public enum State {
      ...
      public void init(AutonEncoderCameraToPeg aectp) {
        ...
        switch (aectp.position) {
          case CENTER:
            ...
            break;
          case BLUE1:
            aectp.addDriveParametersSide(new Point2D(-1.96 /* testing this value */, 0.95),
                -Math.PI / 3, // default
                              // -2.02
          ...
          case BLUE3:
            aectp.addDriveParametersSide(new Point2D(-2.02, -0.80), Math.PI / 3,
                Preferences.getInstance().getDouble("drivetrain.offsetBlue3", 0.3),
                Preferences.getInstance().getDouble("drivetrain.offsetAngleB3", 0.3));
            break;
          case RED1:
            aectp.addDriveParametersSide(new Point2D(-2.02, 0.80), -Math.PI / 3,
                Preferences.getInstance().getDouble("drivetrain.offsetRed1", 0.2),
                Preferences.getInstance().getDouble("drivetrain.offsetAngleR1", -0.15));
            break;
          case RED3:
    ...
    ReadCamera {
      @Override
      public State run(AutonEncoderCameraToPeg aectp) {
        if (Timer.getFPGATimestamp() - aectp.startTime > 0.5) {
          return DriveToPeg;
        }
        System.out.println(SmartDashboard.getNumber("mm peg centerX", 0) / 1000);
        return this;
      }
    ...
    DriveToPeg {
      ...
      public State run(AutonEncoderCameraToPeg aectp) {
        if (aectp.driveTrain.isControllerFinished()) {
          if (aectp.control == Control.NOTHING) {
            return End;
          } else {
            return WaitForGear;
          }
        ...
        switch (aectp.position) {
          case BLUE1:
            aectp.driveTrain.addControllerMotion(aectp.createCameraSpline(
                Preferences.getInstance().getDouble("drivetrain.pegExtraB1", 0.0)));
            break;
          case BLUE3:
            aectp.driveTrain.addControllerMotion(aectp.createCameraSpline(
                Preferences.getInstance().getDouble("drivetrain.pegExtraB3", 0.0)));
            break;
          case CENTER:
            aectp.driveTrain.addControllerMotion(aectp.createCameraSpline(
                Preferences.getInstance().getDouble("drivetrain.pegExtraCenter", 0.0)));
            break;
          case RED1:
            aectp.driveTrain.addControllerMotion(aectp.createCameraSpline(
                Preferences.getInstance().getDouble("drivetrain.pegExtraR1", 0.0)));
            break;
          case RED3:
            aectp.driveTrain.addControllerMotion(aectp.createCameraSpline(
                Preferences.getInstance().getDouble("drivetrain.pegExtraR3", 0.0)));
            break;
        }
        aectp.driveTrain.startMotion();
      }
  ...
  private void addDriveParametersCenter() {
    ...
    driveTrain
        .addControllerMotion(new MotionPathStraight(ZERO, -1.184, MAX_SPEED, MAX_ACCELERATION));
  }
  ...
  private void addDriveParametersSide(Point2D pegPoint, double angle, double offset,
    ...
    double l0 =
        1.0 * (Math.abs(pegPoint.x) - Math.abs(pegPoint.y) / Math.tan(Math.abs(angle))) + offset;
    double l1 = 1.0 * Math.abs(pegPoint.y) / Math.sin(Math.abs(angle));

    driveTrain.addControllerMotion(
        new MotionPathSpline(ZERO, l0, peg, l1, MAX_SPEED, MAX_ACCELERATION, false));
  }
  ...
  private MotionProvider createCameraSpline(double offset) {
    ...
    return new MotionPathSpline(ZERO, controlPoint, peg, controlPoint, PEG_SPEED, PEG_ACCELERATION,
        false);
  }
  ...
  private void addDriveParametersShoot(double pegAngle, double shootAngle) {
    ...
    Point2D displacement =
        new Point2D(Math.cos(pegAngle) * shootDistance + 0.2, Math.sin(pegAngle) * shootDistance);

    Pose atPeg = new Pose(new Point2D(0, 0), pegAngle);
    Pose displacementPose = new Pose(displacement, pegAngle + shootAngle);

    MotionProvider shootDrive = new MotionPathSpline(atPeg, shootDistance / 2, displacementPose,
        shootDistance / 2, MAX_SPEED, MAX_ACCELERATION, true);
