...
public class Kinematics {
	...
	private void evaluateNextPose(double ds) {
		...
		case LimitRotationalAcceleration:
			...
			double omegaAccel = Math
					.sqrt(motion.aMax * Math.abs(MotionProvider.boundAngle(thetaMidpoint - motion.getInitialTheta())));
			double omegaDecel = Math
					.sqrt(motion.aMax * Math.abs(MotionProvider.boundAngle(thetaMidpoint - motion.getFinalTheta())));
